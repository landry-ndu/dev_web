...
/* ---- Letterbox (localStorage) ---- */
const Letterbox = (() => {
  const KEYS = {
    played:    'gb_lb_played',
    want:      'gb_lb_want',
    playing:   'gb_lb_playing',
    followed:  'gb_lb_followed'
  };
  ...
  function toggle(key, gameId) {
    const list = getList(key);
    const idx = list.indexOf(gameId);
    if (idx === -1) list.push(gameId);
    else list.splice(idx, 1);
    saveList(key, list);
    return idx === -1; /* true = ajouté */
  }

  function has(key, gameId) {
    return getList(key).includes(gameId);
  }

  function getStatus(gameId) {
    if (has('playing', gameId))  return 'playing';
    if (has('played',  gameId))  return 'played';
    if (has('want',    gameId))  return 'want';
    return null;
  }

  function setStatus(gameId, status) {
    ['played', 'want', 'playing'].forEach(k => {
      const l = getList(k).filter(id => id !== gameId);
      saveList(k, l);
    });
    if (status) toggle(status, gameId);
  }

  function followGame(gameId, gameName, gameSlug) {
    const followed = getList('followed');
    const exists = followed.find(g => g.id === gameId);
    if (exists) {
      saveList('followed', followed.filter(g => g.id !== gameId));
      return false;
    }
    followed.push({ id: gameId, name: gameName, slug: gameSlug });
    saveList('followed', followed);
    return true;
  }

  function isFollowed(gameId) {
    return !!getList('followed').find(g => g.id === gameId);
  }

  function getFollowed() {
    return getList('followed');
  }

  return { toggle, has, getStatus, setStatus, followGame, isFollowed, getFollowed, getList };
})();
